...
Home
 └─ See your offer
     └─ Offer / eligibility
         ├─ Approved → Amount select (tiers up to limit; over-limit disabled)
...
- **No separate "over-limit" screen.** Per Context Digest assumption #4, over-limit is prevented at input (disabled tiers + inline caption on Amount select), never a post-submit rejection. Building a dedicated over-limit *error* screen would mean the prevention failed — the right fix was to make submitting an invalid amount impossible, not to design a nice error for it.
...
| Offer | — | Skeleton (~650ms, simulates eligibility check) | N/A | Approved banner + CTA | Declined banner (3 reasons) with real next step | — |
| Amount select | Tier chips | — | — | Tier chosen → Continue enabled | — | Tiers above limit disabled + inline caption; Continue disabled until a tier is chosen |
